...
echo "Downloading Server devcommands ${VERSION} from Hexium..."
curl -fsSL -A 'Valheim-native-setup' -o "${tmp_dir}/Server_devcommands.zip" "${DOWNLOAD_URL}"
...
echo "Admin client:    cài mod Server devcommands (JereKuusela) cùng version ${VERSION} qua Gale/Hexium."
